...
from .models import Slider, Banner, Category, SubCategory, MainCategory, Product, Color, Brand, Cart, Coupon_Code, \
    ReviewRating, Address, Order, Refund, Post, BlogCategory, Signup, Author, PostView
...
import stripe
from .forms import ReviewForm, AddressForm, RefundForm, CommentForm, PostForm
...
def generateRecommendation(request):
    ...
    for item in productsAll:
        x = [item.id, item.featured_image, item.product_name, item.Product_information, item.Description, item.price,
             item.Categories]
        y += [x]

    products_df = pd.DataFrame(y, columns=['productId', 'featured_image', 'product_name', 'product_information',
        ...
        if userInput.count() == 0:
            ...
            try:
                ...
                recommender = products_df.loc[
                    products_df['product_name'].isin(recommendation_df.head(5)['productName'].tolist())]
                print(recommender)
                print("helllooooo")
                params = {}
